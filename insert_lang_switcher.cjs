...
const logoutBtnStart = `<button onClick={() => logout()}`;
...
content = content.replace(logoutBtnStart, replacement);

const logoutBtnEnd = `</button>
        </nav>`;

const replacementEnd = `</button>
            </div>
        </nav>`;
content = content.replace(logoutBtnEnd, replacementEnd);
