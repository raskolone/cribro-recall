...
const paths = JSON.parse(fs.readFileSync('yarn_organic_paths.json', 'utf8'));
...
const replacement = `const AIGenerationLoader: React.FC<{ language: 'pl' | 'en'; level: string; logs?: string; currentModel?: string }> = ({ language, level, currentModel }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Step 2: Draw the organic hand-drawn spiral while fading it in
      ...
      // Step 3: Let it sit as a perfectly organized ball for a moment
      // (The yoyo and repeatDelay will handle the loop back)

      // Continuous slow rotation on the spiral to make it feel alive
      // Transform origin is exactly the center of the drawn SVG coordinates
      gsap.to(spiralRef.current, {
        rotation: 360,
        transformOrigin: "350px 150px",
        duration: 30, // Very slow, calm rotation
        ease: "none",
        repeat: -1
      });
        ...
        >
          ...
          <g filter="url(#yarnGlow)">
            <g filter="url(#yarnShadow)">
              ...
              {/* Organized Organic Spiral (Centered) */}
              <path
                ref={spiralRef}
                d={spiralPath}
                stroke="#8BC34A"
