...
const paths = JSON.parse(fs.readFileSync('yarn_center_paths.json', 'utf8'));
...
const replacement = `const AIGenerationLoader: React.FC<{ language: 'pl' | 'en'; level: string; logs?: string; currentModel?: string }> = ({ language, level, currentModel }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Step 2: Draw the spiral while fading it in, simultaneously
      ...
      // Step 3: Keep rotating the spiral a bit to show it's "processing"
      tl.to(spiralRef.current, {
        rotation: 180,
  ...
  return (
    <div ref={containerRef} className="flex flex-col items-center justify-center p-8 text-center w-full max-w-4xl mx-auto my-auto animate-fade-in relative min-h-[420px]">
      <div className="yarn-container relative w-full max-w-[500px] aspect-video mb-4 flex items-center justify-center select-none origin-center overflow-visible">
        ...
        >
          <defs>
            ...
            <filter id="yarnGlow" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="0" stdDeviation="10" floodColor="#7CB342" floodOpacity="0.3" />
            </filter>
          </defs>
