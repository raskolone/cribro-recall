  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-extrabold tracking-tight">Word Sets</h1>
      ...
      {isCreating && (
        <Card className="max-w-md">
          <h2 className="text-xl font-bold mb-4">Create Word Set</h2>
          <form onSubmit={handleCreateSet} className="space-y-4">
            <div>
              <label htmlFor="setName" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Set Name</label>
              <input
                id="setName"
                type="text"
                value={newSetName}
            ...
            <div>
              <label htmlFor="setDescription" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Description (Optional)</label>
              <textarea
                id="setDescription"
                value={newSetDescription}
                onChange={(e) => setNewSetDescription(e.target.value)}
                className="block w-full px-4 py-2 bg-base-100 dark:bg-dark-base-100 border border-base-300 dark:border-dark-base-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200"
                placeholder="e.g., Words for my upcoming trip to Paris"
                rows={3}
              />
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {wordSets.length === 0 ? (
          <p className="text-gray-500 dark:text-gray-400 col-span-full">You haven't created any word sets yet.</p>
        ) : (
          wordSets.map(set => {
            ...
            return (
              <Card key={set.id} className="flex flex-col">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 mt-4">{wordCount} word{wordCount !== 1 ? 's' : ''}</p>
                </div>
