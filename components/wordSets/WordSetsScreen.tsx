  ...
  return (
    <div className="space-y-6">
      ...
      {isCreating && (
        <Card className="max-w-md">
          ...
          <form onSubmit={handleCreateSet} className="space-y-4">
            <div>
              <label htmlFor="setName" className="block text-sm font-bold text-text-faint dark:text-content mb-1">{i18n.t("Set Name")}</label>
              <input
                id="setName"
                type="text"
                value={newSetName}
            ...
            <div>
              <label htmlFor="setDescription" className="block text-sm font-bold text-text-faint dark:text-content mb-1">{i18n.t("Description (Optional)")}</label>
              <textarea
                id="setDescription"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {wordSets.length === 0 ? (
          <p className="text-text-mute dark:text-text-2 col-span-full">{i18n.t("You haven't created any word sets yet.")}</p>
        ) : (
          wordSets.map(set => {
            ...
            return (
              <Card key={set.id} className="flex flex-col">
                <div className="flex-1">
                  ...
                  {set.description && <p className="text-sm text-text-faint dark:text-text-2 mt-1">{set.description}</p>}
                  <p className="text-sm text-text-mute mt-4">{wordCount}  {i18n.t("word")}{wordCount !== 1 ? 's' : ''}</p>
                </div>
                <div className="mt-6 flex justify-end gap-4">
                  <button
                    onClick={() => setEditingSetId(set.id)}
                    className="text-primary hover:text-accent-soft text-sm font-bold transition-colors"
                  >
