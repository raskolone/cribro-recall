    ...
    return (
      ...
      >
        ...
        {messages.length === 0 && viewMode === 'chat' && (
          <div className="flex flex-col items-center text-center space-y-2 py-4 sm:py-6 max-w-xl mx-auto animate-fadeIn">
        ...
        >
          ...
          {viewMode === 'history' ? (
            ...
            <>
                ...
                <div className="p-3.5 sm:p-4 border-b border-slate-200/70 dark:border-white/5 space-y-2.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {[
                      ...
                      'Zaproponuj scenariusz lekcji na 60 minut',
                      'Utwórz konspekt do Prezentacji Live',
                    ].map((promptText, sIdx) => (
                      <button
                        key={sIdx}
              ...
              {/* Stan 2: Strumień wiadomości w trwającej rozmowie */}
              {messages.length > 0 && (
                <div
                  ref={scrollRef}
                  className="flex-1 overflow-y-auto p-3.5 sm:p-5 space-y-3.5 scroll-smooth pr-2"
                >
                  {messages.map((message, idx) => {
                    ...
                    // Sanityzacja wycieków z modelu
                    const { cleanedText, followUps: inlineFollowups } =
                      message.role === 'assistant'
                        ? extractFollowUpsAndCleanText(message.text)
                        : { cleanedText: message.text, followUps: [] };
                        ...
                        {message.role === 'user' ? (
                          ...
                          <div className="flex items-start gap-2.5 max-w-[98%] sm:max-w-[92%] mr-auto">
                            ...
                            <div className="flex-1 min-w-0 space-y-2">
                              <div className="bg-ink-2/95 border border-line-strong text-text p-3.5 sm:p-4 rounded-2xl rounded-tl-xs shadow-ambient-sm space-y-2.5">
                                <div className="flex items-center justify-between border-b border-line pb-1.5 text-[10px] text-text-mute font-mono flex-wrap gap-1">
                                  ...
                                  <div className="flex items-center gap-2 ml-auto">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => copyMessage(idx, cleanedText)}
                                      className="p-1 hover:text-text-hi text-text-mute rounded transition-colors flex items-center gap-1 cursor-pointer text-[9px]"
                                ...
                                {/* Główna treść odpowiedzi Markdown — wyłącznie oczyszczony tekst */}
                              ...
                              {/* Wyekstrahowane follow-upy WYŁĄCZNIE jako klikalne pigułki pod odpowiedzią asystenta */}
                              {effectiveFollowups && effectiveFollowups.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 pt-1 pl-1 items-center">
                                  <span className="text-[10px] font-bold text-text-mute uppercase tracking-wider font-mono mr-1 flex items-center gap-1">
                                    <Sparkles size={11} className="text-primary" /> Sugestie:
                                  </span>
                                  {effectiveFollowups.map((item, fIdx) => (
                                    <button
                                      key={fIdx}
                                      type="button"
                                      onClick={() => {
                                      ...
                                      disabled={isThinking}
                                      className="bg-primary/15 hover:bg-primary/25 text-primary border border-primary/40 rounded-lg px-3 py-1.5 text-xs font-medium transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer shadow-sm disabled:opacity-50"
                                    >
                                      <span>{item}</span>
