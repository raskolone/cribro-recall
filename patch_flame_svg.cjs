  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      gsap.to(outerFlameRef.current, {
        ...
        repeat: -1,
        yoyo: true,
        ease: "rough({ template: power1.inOut, strength: 0.5, points: 10, randomize: true })",
        repeatRefresh: true
      });
...
const oldFlameRegex = /const AnimatedFlame = \(\) => \{[\s\S]*?return \([\s\S]*?<\/span>[\s\S]*?\);[\s\S]*?\};/;

if (code.match(oldFlameRegex)) {
  code = code.replace(oldFlameRegex, svgFlameComponent);
} else {
  console.log("Could not find old AnimatedFlame component to replace.");
}
